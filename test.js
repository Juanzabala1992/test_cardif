...
cat <<'EOF' | while read -r ip port; do
55.19.78.110 9090
55.19.78.110 9091
55.19.78.110 9092
55.19.78.110 9093
...
EOF
  echo "===== $ip:$port ====="
  timeout 3 nc -vz -w 2 "$ip" "$port" 2>&1
  echo
done

cat <<'EOF' | while read -r ip port; do 55.19.78.110 9090 55.19.78.110 9091 55.19.78.110 9092 55.19.78.110 9093 10.229.253.110 9090 10.229.253.110 9091 10.229.253.110 9092 10.229.253.110 9093 EOF echo "===== $ip:$port =====" timeout 3 nc -vz -w 2 "$ip" "$port" 2>&1 echo done
